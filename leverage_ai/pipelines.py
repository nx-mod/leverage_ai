...
def pipeline_code(idea: str, state: Dict[str, Any], noexec: bool = False, 
                  mem: Optional[Dict[str, Any]] = None, settings: Optional[Dict[str, Any]] = None):
    """Handle code generation requests with full pipeline."""
    logger.debug("Code pipeline")
    settings = settings or {}
    sandbox = settings.get("sandbox_code", False)
    auto_run = settings.get("auto_run_code", False)
    ...
    if word_count < 50:
        ...
        if cached_impl:
            ...
            if blocks:
                ...
                for p in paths:
                    print(f"  [saved] {p}")
                install_deps(paths, sandbox, auto_run)
                show_files(paths)
                try_run(paths, idea, noexec, sandbox, auto_run)
