...
from leverage_ai.provider_settings import (
    load_provider_settings, is_disabled as is_provider_disabled, has_configured_key,
)
from leverage_ai.chain_settings import load_chain_settings, filter_chain
...
_CHAIN_NAME_BY_ID = {id(v): k for k, v in CHAIN_REGISTRY.items()}


def _chain_name_for(provider_chain: List[str]) -> Optional[str]:
    """Identity-match a chain list against the named registry in config.py.

    This lets /models per-chain disables apply without every pipelines.py
    call site needing to pass an explicit chain name string.
    """
    return _CHAIN_NAME_BY_ID.get(id(provider_chain))
...
def heal_chain(chain: List[str], depleted: List[str]) -> List[str]:
    ...
    alive = [
        ...
        if p not in depleted and not is_provider_disabled(provider_settings, p) and has_configured_key(p)
    ]
...
def run_stage(provider_chain: List[str], prompt: str, max_tokens: int, state: Dict[str, Any],
    ...
    # Apply /providers global disables + live key-presence, and /models
    # per-chain disables, before the existing depleted-provider filtering.
    provider_settings = load_provider_settings()
    chain_settings = load_chain_settings()
    chain_name = _chain_name_for(provider_chain)

    available_chain = [
        p for p in provider_chain
        if not is_provider_disabled(provider_settings, p) and has_configured_key(p)
    ]
    if not available_chain:
        # Fall back to key-presence filtering alone, in case the user
        # disabled every provider that happens to have a key configured -
        # better to try something than refuse outright.
        available_chain = [p for p in provider_chain if has_configured_key(p)]
    if not available_chain:
        available_chain = provider_chain  # last resort: try everything, let errors surface naturally

    if chain_name:
        available_chain = filter_chain(chain_settings, chain_name, available_chain)

    # Heal depleted providers
    depleted = state.get("depleted", [])
    active_chain = [p for p in available_chain if p not in depleted]
